...
public:
    ...
    void Shutdown() {
        ...
        if (m_keyboardThread && m_keyboardThread->joinable()) {
            ...
            // GetThreadId requires Windows >= Vista, consider alternative if compatibility needed
            DWORD threadId = GetThreadId(m_keyboardThread->native_handle());
            if (threadId != 0) {
                PostThreadMessage(threadId, WM_QUIT, 0, 0);
                m_keyboardThread->join();
            } else {
                // Fallback if GetThreadId is not available or fails
                // May need a more robust shutdown mechanism for the keyboard thread
                std::cerr << "Warning: Could not get Keyboard thread ID for shutdown." << std::endl;
            }
        }
    }
...
void KeyboardHookThreadProc() {
    ...
    UndownUnlock::WindowsHook::KeyboardHook::Initialize(); // Removed check for success
...
extern "C" __declspec(dllexport) bool IsHookActive() {
    // Return a simplified status, as we can't access DXHookCore's internals directly here
    return !g_shutdown;
} 